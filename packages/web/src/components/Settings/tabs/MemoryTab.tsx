...
export function MemoryTab({
    ...
    return (
        <div>
            ...
            >
                Control how Kestrel retrieves context from conversation memory and workspace
                knowledge.
            </p>
            <div
                style={{
            ...
            >
                <div>
                    ...
                    <div style={{ fontSize: '0.7rem', color: '#444', marginTop: '4px' }}>
                        Augment responses with relevant past context
                    </div>
                </div>
            ...
            {ragEnabled && (
                <div
                    style={{ borderTop: '1px solid #1a1a1a', paddingTop: '20px', marginTop: '4px' }}
                >
                    <SliderField
                        label="Retrieval Count (top-k)"
                        value={ragTopK}
                        onChange={(v) => setRagTopK(Math.round(v))}
                        min={1}
                        max={20}
                        step={1}
                        format={(v) => `${v} chunks`}
                    ...
                    >
                        <div style={{ fontSize: '0.7rem', color: '#666', lineHeight: 1.6 }}>
                            <span style={{ color: '#00f3ff' }}>top-k = {ragTopK}</span> — chunks
                            retrieved per query
                            <br />
                            <span style={{ color: '#00f3ff' }}>
                                threshold = {(ragMinSimilarity * 100).toFixed(0)}%
                            </span>{' '}
                            — minimum similarity
                        </div>
                    </div>
