...
export function CapabilitiesTab({
    ...
    const toggleCapability = async (capId: string) => {
        ...
        if (!cap.installed) {
            ...
            try {
                await request(`/workspaces/${workspaceId}/capabilities/${capId}/install`, {
                    method: 'POST',
                });
                setCapabilities((prev) =>
                    prev.map((c) =>
                        c.id === capId ? { ...c, installed: true, enabled: true } : c,
                    ),
                );
            } catch (err: any) {
            ...
            try {
                ...
                setCapabilities((prev) =>
                    prev.map((c) => (c.id === capId ? { ...c, enabled: newEnabledState } : c)),
                );
                ...
                setCapabilities((prev) =>
                    prev.map((c) => (c.id === capId ? { ...c, enabled: cap.enabled } : c)),
                );
    ...
    return (
        <div>
            ...
            >
                Enable or disable deep system integrations. These are highly privileged capabilities
                built directly into Kestrel's core.
            </p>
            ...
            >
                {capabilities.map((cap) => (
                    ...
                    >
                        ...
                        <div
                            style={{ fontSize: '0.75rem', color: '#888', lineHeight: 1.5, flex: 1 }}
                        >
                            {cap.description}
                        </div>
