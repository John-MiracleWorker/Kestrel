...
const STATE_CONFIG: Record<
    OrbState,
    { color: string; glow: string; animation: string; label: string }
> = {
    idle: {
        color: '#00f3ff',
    ...
    executing: {
        ...
        animation: 'orb-flash 0.6s ease-in-out infinite',
        label: 'EXECUTING',
    },
    escalated: {
        color: '#f97316',
        glow: 'rgba(249, 115, 22, 0.5)',
        animation: 'orb-escalated 1.5s ease-in-out infinite',
        label: 'CLOUD',
    },
    error: {
        color: '#ff0055',
        glow: 'rgba(255, 0, 85, 0.5)',
        animation: 'orb-error 0.8s ease-in-out infinite',
        label: 'ERROR',
    },
    disconnected: {
        color: '#6b7280',
...
export function StatusOrb(props: StatusOrbProps) {
    const state = useMemo(
        () => deriveState(props),
        [props.isConnected, props.isStreaming, props.toolStatus, props.wasEscalated],
    );
