        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {blocks.map((block, i) => {
                switch (block.type) {
                    ...
                    case 'chart':
                        return (
                            <ChartBlock key={i} type={block.chartType || 'bar'} data={block.data} />
                        );
                    case 'table':
...
function markdownToHtml(md: string): string {
    ...
    let html = processed
        ...
        .replace(
            /^#### (.+)$/gm,
            '<h4 style="margin:12px 0 4px;font-size:0.85rem;color:var(--text-primary);font-weight:600">$1</h4>',
        )
        .replace(
            /^### (.+)$/gm,
            '<h3 style="margin:14px 0 4px;font-size:0.9rem;color:var(--text-primary);font-weight:600">$1</h3>',
        )
        .replace(
            /^## (.+)$/gm,
            '<h2 style="margin:16px 0 6px;font-size:0.95rem;color:var(--text-primary);font-weight:700">$1</h2>',
        )
        .replace(
            /^# (.+)$/gm,
            '<h1 style="margin:16px 0 8px;font-size:1.05rem;color:var(--text-primary);font-weight:700">$1</h1>',
        )
        ...
        .replace(
            /`([^`]+)`/g,
            '<code style="background:rgba(0,243,255,0.08);color:var(--accent-cyan);padding:1px 5px;border-radius:3px;font-family:var(--font-mono);font-size:0.85em">$1</code>',
        )
        ...
        .replace(
            /\[([^\]]+)\]\(([^)]+)\)/g,
            '<a href="$2" target="_blank" rel="noopener" style="color:var(--accent-cyan);text-decoration:underline">$1</a>',
        )

        // Bullet lists (- or * at start of line)
        .replace(
            /^(?:[-*]) (.+)$/gm,
            '<li style="margin-left:16px;padding:2px 0;list-style:disc outside">$1</li>',
        )

        // Numbered lists
        .replace(
            /^(\d+)\. (.+)$/gm,
            '<li style="margin-left:16px;padding:2px 0;list-style:decimal outside" value="$1">$2</li>',
        )

        // Wrap consecutive <li> in <ul>/<ol>
        .replace(
            /((?:<li[^>]*>.*<\/li>\n?)+)/g,
            '<ul style="margin:4px 0;padding-left:8px">$1</ul>',
        )
