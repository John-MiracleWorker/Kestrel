...
export function ClinicalPanel({ isVisible, onClose }: ClinicalPanelProps) {
    ...
    const [newAuth, setNewAuth] = useState<Partial<PriorAuth>>({
        status: 'Pending',
        date: new Date().toISOString().split('T')[0],
    });
    ...
    const handleAddReferral = (e: React.FormEvent) => {
        e.preventDefault();
        setReferrals([
            { ...newReferral, id: Math.random().toString(36).substr(2, 9) } as Referral,
            ...referrals,
        ]);
        setIsAdding(false);
    ...
    const handleAddAuth = (e: React.FormEvent) => {
        e.preventDefault();
        setAuths([
            { ...newAuth, id: Math.random().toString(36).substr(2, 9) } as PriorAuth,
            ...auths,
        ]);
        setIsAdding(false);
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
                    ...
                    >
                        ...
                        >
                            <input
                                ...
                                onChange={(e) =>
                                    setNewReferral({ ...newReferral, patientName: e.target.value })
                                }
                                style={inputStyle}
                            />
                            <input
                                required
                                placeholder="Specialty (e.g., Dentistry)"
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    setNewReferral({ ...newReferral, location: e.target.value })
                                }
                                style={inputStyle}
                            />
                            <input
                    ...
                    >
                        ...
                        >
                            <input
                                ...
                                onChange={(e) =>
                                    setNewAuth({ ...newAuth, patientName: e.target.value })
                                }
                                style={inputStyle}
                            />
                            <input
                                ...
                                onChange={(e) =>
                                    setNewAuth({ ...newAuth, procedure: e.target.value })
                                }
                                style={inputStyle}
                            />
                            <input
                                placeholder="Order Number"
                                value={newAuth.orderNumber || ''}
                                onChange={(e) =>
                                    setNewAuth({ ...newAuth, orderNumber: e.target.value })
                                }
                                style={inputStyle}
                            />
                            <input
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {activeTab === 'referrals' && referrals.length === 0 && (
                        <div style={emptyStyle}>No active referrals.</div>
                    )}
                    {activeTab === 'referrals' &&
                        referrals.map((r) => (
                            <div key={r.id} style={cardStyle}>
                                ...
                                >
                                    <select
                                        value={r.status}
                                        onChange={(e) =>
                                            updateReferralStatus(r.id, e.target.value as any)
                                        }
                                        style={selectStyle(
                                            r.status === 'Completed'
                                                ? 'var(--accent-green)'
                    ...
                    {activeTab === 'auths' && auths.length === 0 && (
                        <div style={emptyStyle}>No active prior authorizations.</div>
                    )}
                    {activeTab === 'auths' &&
                        auths.map((a) => (
                            <div key={a.id} style={cardStyle}>
                                ...
                                >
                                    <select
                                        value={a.status}
                                        onChange={(e) =>
                                            updateAuthStatus(a.id, e.target.value as any)
                                        }
                                        style={selectStyle(
                                            a.status === 'Approved'
                                                ? 'var(--accent-green)'
