    ...
    typedApp.post(
        ...
        async (req, reply) => {
            ...
            try {
                ...
                try {
                    if (brainClient.isLocalMode()) {
                        const existing = await brainClient.listWorkspaces(user.id);
                        if (existing[0]?.id) {
                            workspaces = [{ id: existing[0].id, role: 'owner' }];
                        }
                    } else {
                        const ws = await brainClient.createWorkspace(
                            user.id,
