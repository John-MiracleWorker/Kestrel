...
def test_primary_queue_continues_when_start_and_first_reconciliation_fail(
    ...
    assert _wait_until(
        lambda: manager.state.get_run(queued_first.run_id).status == "failed"
    )
    assert _wait_until(
        lambda: manager.state.get_run(queued_second.run_id).status == "completed"
    )
...
def test_scheduler_task_close_failure_fails_worker_before_terminal_publication(
    ...
    event_types = {
        event["type"] for event in manager.state.list_run_steps(run.run_id)
    }
...
def test_public_subagent_close_failure_fails_worker_before_terminal_publication(
    ...
    assert _wait_until(
        lambda: manager.state.get_subagent_run(str(created["subagent_id"])).status
        == "failed"
    )
