...
def test_python_and_web_release_metadata_stay_aligned() -> None:
    ...
    assert PUBLISHED_RELEASE == "0.5.0"
    assert "nested-memvid-agent 0.5.0 release" in result.stdout
    assert "stable release metadata v0.5.0" in result.stdout
    assert "kestrel-web 0.5.0" in result.stdout
...
def test_deployment_docs_cover_release_and_memory_operations() -> None:
    ...
    assert (
        ...
        "v0.5.0/install.sh | bash"
    ) in deployment
    assert "`v0.5.0` is the current stable release" in deployment
    assert "release artifacts are not claimed to exist" in deployment
    assert "releases/download/v0.5.0/install.sh" in deployment
...
def test_desktop_workspace_is_exact_pinned_and_emits_hardened_main() -> None:
    ...
    assert "readinessTimeoutMs: 90_000" in main_source
    assert "readinessTimeoutMs: 15_000" not in main_source
    assert "readinessTimeoutMs: 40_000" not in main_source
