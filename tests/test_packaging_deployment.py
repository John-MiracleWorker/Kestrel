...
def test_python_and_web_release_metadata_stay_aligned() -> None:
    ...
    assert PUBLISHED_RELEASE == "0.5.8"
    assert "nested-memvid-agent 0.5.8 release" in result.stdout
    assert "stable release metadata v0.5.8" in result.stdout
    assert "kestrel-web 0.5.8" in result.stdout
...
def test_deployment_docs_cover_release_and_memory_operations() -> None:
    ...
    assert (
        ...
        "v0.5.8/install.sh | bash"
    ) in deployment
    assert "`v0.5.8` is the current stable release" in deployment
    assert "release artifacts are not claimed to exist" in deployment
    assert "releases/download/v0.5.8/install.sh" in deployment
