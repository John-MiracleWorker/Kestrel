...
def _write_payload(
    root: Path,
    *,
    wheel_metadata_version: str = "0.4.8",
    sdist_metadata_version: str = "0.4.8",
    sbom_version: str = "0.4.8",
) -> None:
    wheel = root / "nested_memvid_agent-0.4.8-py3-none-any.whl"
    with zipfile.ZipFile(wheel, "w") as archive:
        archive.writestr(
            "nested_memvid_agent-0.4.8.dist-info/METADATA",
            "Metadata-Version: 2.4\n"
            "Name: nested-memvid-agent\n"
            f"Version: {wheel_metadata_version}\n",
        )
    sdist = root / "nested_memvid_agent-0.4.8.tar.gz"
    package_info = (
        f"Metadata-Version: 2.4\nName: nested-memvid-agent\nVersion: {sdist_metadata_version}\n"
    ).encode()
    with tarfile.open(sdist, "w:gz") as archive:
        member = tarfile.TarInfo("nested_memvid_agent-0.4.8/PKG-INFO")
...
def test_release_payload_verifier_covers_every_artifact_and_detects_tampering(
    ...
    report = verify_release_payload(tmp_path, expected_version="v0.4.8")

    assert report["verified"] is True
    assert report["artifact_count"] == 5
    assert report["requirement_count"] == 1
    assert report["distribution"] == "nested-memvid-agent"
    assert report["version"] == "0.4.8"

    (tmp_path / "install.sh").write_bytes(b"tampered")
    with pytest.raises(ValueError, match="SHA-256 mismatch for install.sh"):
        verify_release_payload(tmp_path, expected_version="0.4.8")


@pytest.mark.parametrize(
    ("kwargs", "message"),
    [
        ({"wheel_metadata_version": "0.4.9"}, "wheel METADATA version mismatch"),
        ({"sdist_metadata_version": "0.4.9"}, "sdist PKG-INFO version mismatch"),
        ({"sbom_version": "0.4.9"}, "CycloneDX Kestrel component identity mismatch"),
    ],
)
...
def test_release_payload_verifier_rejects_filename_identity_drift(tmp_path: Path) -> None:
    _write_payload(tmp_path)
    wheel = tmp_path / "nested_memvid_agent-0.4.8-py3-none-any.whl"
    wheel.rename(tmp_path / "nested_memvid_agent-0.4.9-py3-none-any.whl")
...
def test_pypi_partial_recovery_skips_only_exact_existing_files(tmp_path: Path) -> None:
    _write_payload(tmp_path)
    wheel = tmp_path / "nested_memvid_agent-0.4.8-py3-none-any.whl"
    sdist = tmp_path / "nested_memvid_agent-0.4.8.tar.gz"
    wheel_record = {
        "filename": wheel.name,
        "digests": {"sha256": hashlib.sha256(wheel.read_bytes()).hexdigest()},
        "yanked": False,
    }
    remote = {"info": {"version": "0.4.8"}, "urls": [wheel_record]}

    assert plan_pypi_files(tmp_path, remote, expected_version="0.4.8") == [sdist]
    exact = {
        "info": {"version": "0.4.8"},
        "urls": [
            wheel_record,
    ...
    assert plan_pypi_files(tmp_path, exact, expected_version="0.4.8") == []
