    ...
    def put_turn(
        *,
        ...
        for role, content in (
            ...
            source_uri = (
                f"agent_runtime://sessions/expiry-session/turns/{turn_id}/{role}"
            )
            memory.put(
                MemoryRecord(
                    id=f"{turn_id}-{role}",
...
def test_internal_turns_never_replay_as_native_user_transcript(tmp_path: Path) -> None:
    ...
    assert {record.metadata.get("transcript_scope") for record in internal_records} == {
        "internal"
    }
...
def test_same_session_id_never_replays_transcript_across_primary_and_channel_scopes(
    ...
    primary_native = [
        ...
        if "scope-f813" in message.content
...
def test_only_gate_validated_policy_memory_retains_system_priority(tmp_path: Path) -> None:
    ...
    compile_event = next(
        event for event in event_log.tail() if event.type == "context.compile"
    )
