...
def test_repair_scheduler_hands_off_only_bounded_receipt_artifacts(tmp_path: Path) -> None:
    ...
    validation_artifact = dict(
        (persisted["Validate repair"].result or {})["repair_artifact"]
    )
    ...
    commit_artifact = dict(
        (persisted["Commit reviewed repair"].result or {})["repair_artifact"]
    )
...
def test_approved_repair_scheduler_flow_binds_real_validation_and_review_receipts(
    ...
    review_artifact = (persisted["Review repair before commit"].result or {})[
        "repair_artifact"
    ]
    ...
    worktree = Path((persisted["Commit reviewed repair"].result or {})["worker_isolation"]["workspace"])
