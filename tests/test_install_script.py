...
def test_installer_user_launcher_transaction_commits_managed_shim(
    ...
    assert result.returncode == 0, result.stderr
    assert shim.is_file()
    assert "KESTREL_MANAGED_COMMAND_SHIM_V1" in shim.read_text(encoding="utf-8")
...
def test_installer_next_steps_use_only_the_product_facade() -> None:
    text = INSTALL.read_text(encoding="utf-8")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "KESTREL_WHEEL_URL",
            "https://replacement.invalid/nested_memvid_agent-0.4.11-py3-none-any.whl",
...
def test_staged_release_installer_rejects_source_overrides(
    ...
    staged = _stage_release_installer(
        tmp_path / "install.sh",
        release_base="https://example.invalid/releases/download/v0.4.11",
        release_sha=TEST_RELEASE_SHA,
        version="0.4.11",
    )
...
def test_staged_release_installer_rejects_moved_tag(tmp_path: Path) -> None:
    ...
    subprocess.run(["git", "tag", "v0.4.11", original_sha], cwd=source, check=True)
    ...
    subprocess.run(["git", "tag", "-f", "v0.4.11"], cwd=source, check=True)
    staged = _stage_release_installer(
        tmp_path / "install.sh",
        release_base="https://example.invalid/releases/download/v0.4.11",
...
def test_source_installer_cannot_enter_unbound_release_artifact_mode(
    ...
    result = _run_install(
        env={
            ...
            "KESTREL_WHEEL_URL": (f"{release_base}/nested_memvid_agent-0.4.11-py3-none-any.whl"),
