...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "KESTREL_WHEEL_URL",
            "https://replacement.invalid/nested_memvid_agent-0.4.5-py3-none-any.whl",
...
def test_staged_release_installer_rejects_source_overrides(
    ...
    staged = _stage_release_installer(
        tmp_path / "install.sh",
        release_base="https://example.invalid/releases/download/v0.4.5",
        release_sha=TEST_RELEASE_SHA,
        version="0.4.5",
    )
...
def test_staged_release_installer_rejects_moved_tag(tmp_path: Path) -> None:
    ...
    subprocess.run(["git", "tag", "v0.4.5", original_sha], cwd=source, check=True)
    ...
    subprocess.run(["git", "tag", "-f", "v0.4.5"], cwd=source, check=True)
    staged = _stage_release_installer(
        tmp_path / "install.sh",
        release_base="https://example.invalid/releases/download/v0.4.5",
...
def test_source_installer_cannot_enter_unbound_release_artifact_mode(
    ...
    result = _run_install(
        env={
            ...
            "KESTREL_WHEEL_URL": (f"{release_base}/nested_memvid_agent-0.4.5-py3-none-any.whl"),
