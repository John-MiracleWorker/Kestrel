    ...
    *,
    ...
    replacements = {
        ...
        'KESTREL_REF="${KESTREL_REF:-main}"': (
            f'KESTREL_REF="${{KESTREL_REF:-{release_ref}}}"'
        ),
...
def test_staged_release_installer_uses_verified_locked_artifacts(tmp_path: Path) -> None:
    ...
    assert f"verify checkout HEAD equals immutable release commit {TEST_RELEASE_SHA}" in result.stdout
...
def test_staged_release_installer_rejects_moved_tag(tmp_path: Path) -> None:
    ...
    subprocess.run(
        ["git", "commit", "-q", "-m", "move tag target"], cwd=source, check=True
    )
...
def test_source_installer_cannot_enter_unbound_release_artifact_mode(
    ...
    result = _run_install(
        env={
            ...
            "KESTREL_WHEEL_URL": (
                f"{release_base}/nested_memvid_agent-0.4.0-py3-none-any.whl"
            ),
...
def test_install_script_launches_server_detached_and_checks_health() -> None:
    ...
    assert main_body.index("finalize_release_install_transaction") < main_body.index(
    ...
    assert main_body.index("finish_post_commit_maintenance") < main_body.index(
        "start_server_detached"
    )
...
def test_failed_server_health_stops_tracked_supervisor_and_child(tmp_path: Path) -> None:
    ...
    assert result.returncode != 0
    assert "filesystem install committed" in result.stderr
