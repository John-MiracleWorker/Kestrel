...
`v0.5.1` is the current stable release for the supported local/private deployment profile. This
...
use the published container image for those platforms. The `v0.5.1`
...
```bash
python -m pip install "nested-memvid-agent[memvid,server,mcp,keyring]==0.5.1"
```
...
```powershell
gh release download v0.5.1 --repo John-MiracleWorker/Kestrel --dir kestrel-release-v0.5.1
python scripts/verify_release_payload.py kestrel-release-v0.5.1 --expected-version v0.5.1
Set-Location kestrel-release-v0.5.1
...
```bash
mkdir kestrel-release-v0.5.1
gh release download v0.5.1 --repo John-MiracleWorker/Kestrel --dir kestrel-release-v0.5.1
for artifact in kestrel-release-v0.5.1/*; do
...
```bash
curl -fsSL https://github.com/John-MiracleWorker/Kestrel/releases/download/v0.5.1/install.sh | KESTREL_START_SERVER=1 KESTREL_OPEN_BROWSER=1 bash
```
...
For a published `v0.5.1` or later release, the Linux ARM64 installer fallback is the
public GHCR image. The same tag is a two-platform index for `linux/amd64` and
`linux/arm64`:

```bash
docker pull --platform linux/arm64 ghcr.io/john-miracleworker/kestrel:v0.5.1
docker run --rm --platform linux/arm64 \
  ghcr.io/john-miracleworker/kestrel:v0.5.1 \
