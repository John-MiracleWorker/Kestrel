...
`v0.4.11` is the current stable release for the supported local/private deployment profile. This
...
Native Windows users should use the checksummed PowerShell diagnostic and exact wheel instead of
...
itself the published release channel.
...
After repository immutable releases are enabled, the GitHub release workflow publishes the universal wheel, source distribution,
version-pinned installer, hash-locked default dependency manifest,
CycloneDX SBOM, an identity-bound OCI image digest record, and `SHA256SUMS`. The digest
record binds the exact repository, tag, commit, image index, and two platform manifests; a
published immutable-release rerun verifies it and performs no registry writes. Draft publication
rejects unknown asset names and byte-compares every final asset with the local payload before the
release becomes immutable. The dependency audit and SBOM are generated
from the same isolated wheel environment described by `requirements-release.txt`,
not from the development/test environment.

After a trusted reviewer is configured on the `pypi` environment and the exact pending publisher is
registered, a separate approval-protected job publishes exactly the verified wheel and sdist to PyPI
through short-lived OIDC Trusted Publishing. For retry recovery,
it queries the version first and accepts an existing file only when both filename and SHA-256 are
exact, uploading only a missing wheel or sdist and failing on conflicting or unknown files. No
long-lived PyPI token is stored in GitHub. Once that job succeeds, a direct package install is:

```bash
python -m pip install "nested-memvid-agent[memvid,server,mcp,keyring]==0.4.11"
```
...
The report checks Git, Python 3.11-3.13, WSL2 distro version/architecture/guest prerequisites, and
Docker Desktop engine reachability with bounded probes. `Bootstrap` only prints commands for the
selected ready path and sets `mutation_performed` to false. The operator must review and execute
those commands separately. It never invokes Windows Features, `wsl --install`, a package manager,
Docker installation, Python installation, Git installation, `pip install`, `docker pull`, or the
Bash installer itself.

Supported choices are:

- `NativeWheel`: a supported 64-bit Python interpreter installs the exact published wheel.
- `WSL2`: an existing x86_64 WSL2 distribution with Git and Python runs the Bash installer inside
  Linux, never through Git Bash.
...
```bash
mkdir kestrel-release-v0.4.11
gh release download v0.4.11 --repo John-MiracleWorker/Kestrel --dir kestrel-release-v0.4.11
for artifact in kestrel-release-v0.4.11/*; do
...
```bash
curl -fsSL https://github.com/John-MiracleWorker/Kestrel/releases/download/v0.4.11/install.sh | KESTREL_START_SERVER=1 KESTREL_OPEN_BROWSER=1 bash
```
...
For a published `v0.4.11` or later release, the Linux ARM64 installer fallback is the
public GHCR image. The same tag is a two-platform index for `linux/amd64` and
`linux/arm64`:

```bash
docker pull --platform linux/arm64 ghcr.io/john-miracleworker/kestrel:v0.4.11
docker run --rm --platform linux/arm64 \
  ghcr.io/john-miracleworker/kestrel:v0.4.11 \
