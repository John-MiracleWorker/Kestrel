...
Capsule summaries produce `LearningSignal` objects and preview Nested Learning decisions. Applying those decisions is separate: `capsule.apply` requires auto-consolidation config and approval before writing. Automatic consolidation and compaction are off by default and dry-run by default. Capsules cannot activate policy memory; activation is a separate `memory.policy_promote` operation requiring explicit instruction, high validation, repeated evidence, config enablement, exact-call human approval, and durable result attestation.
