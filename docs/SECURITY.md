...
The Secret Broker lets a trusted backend/UI flow collect values through `POST /api/secrets` and returns metadata only: name, purpose, `secret://...` reference, configured state, validation status, timestamps, and a salted non-reversible fingerprint. No GET route returns raw values. Before that API publishes a secret, Kestrel serializes the transition against local MCP stdio launch and closes every registered stdio session; publication fails with `mcp_stdio_quiesce_failed` when session teardown cannot be verified. Repair-signing-key creation and rotation use the same gate. The default local JSON vault uses cross-process locking and atomic replacement from an owner-only temporary file so a partially written or permissively exposed vault is never the live file. On startup and before reads, an existing vault is validated through a no-follow descriptor and repaired to mode `0600`; symlinked, hard-linked, non-regular, or foreign-owned vault and lock files fail closed before target mutation. Native Windows exposes NTFS ACLs rather than meaningful POSIX mode bits, so deployments requiring strict account separation should select a usable keyring/DPAPI-backed implementation instead of relying on the JSON vault. The JSON vault assumes a non-shared, single-user machine. The published installer and container image include the `keyring` package through the dedicated `keyring` extra. Set `NEST_AGENT_SECRET_BACKEND=keyring` or `--secret-backend keyring` to store raw values in the OS keyring while keeping only metadata in the JSON vault. Explicit `keyring` selection fails closed when the package or a usable OS keyring backend is unavailable; it never silently downgrades to plaintext JSON. The package does not create an OS credential service: Linux requires an unlocked Secret Service session, and stock headless containers remain on JSON unless an operator deliberately supplies a compatible authenticated service. Never reuse a populated JSON vault as keyring metadata. Migrate to a new empty metadata path, rotate/re-enter and validate the secrets, then retire the old raw vault. Shared environments should back the same broker contract with an OS keychain or managed vault such as macOS Keychain, Linux Secret Service via `keyring`, Windows DPAPI, or a managed secret store.

The configured store is a policy boundary even when it has an innocuous custom filename. File, shell, patch, repair, and Git tools reject the JSON vault, its lock and atomic-write temporary files, and same-inode aliases. Git diff/show/export/status inspect affected paths before rendering, reject historical or deleted vault paths, disable repository-configured external diff/text-conversion processes, and override repository fsmonitor execution. Moving a vault outside the workspace only reduces accidental discovery; it is not OS containment against arbitrary same-account code. An OS keyring improves at-rest storage but is also not process isolation: same-account Python, tests, linters, Codex, or MCP processes can call the keyring API themselves. Agent-invoked `test.run`, `lint.run`, repair validation, and `codex.exec` therefore always require `NEST_AGENT_VALIDATION_CONTAINER_IMAGE` set to a preloaded digest-pinned image and use the default-deny OCI snapshot path described below; there is no host fallback. This image receives no host credentials or network. The separate `codex-cli` response provider has no contained adapter and fails closed when raw JSON vault bytes, keyring secret records, or repair signing/receipt trust material exist. An empty keyring metadata file with no secret records does not trigger that host-process fence and is never used to enumerate the OS keyring. Snapshot-dependent repair status/review/rollback and repair-branch commit gates retain the raw-JSON fence because Git-backed candidate fingerprinting must not inspect vault bytes. CLI worker construction primes the same broker/redaction registry and passes one resolver to the run manager and MCP manager; read-only observer commands deliberately skip broker construction so status inspection cannot create, harden, or prompt for secret storage.
...
MCP manifests must reference secret material through `secret_env`, where each target process variable maps to a host environment variable or `secret://...` reference. Raw secret-looking keys in MCP `env` are rejected, API responses redact `secret_env`, and values are resolved into the child process environment only at launch. Every live stdio server, including manually configured servers, requires explicit connect approval bound to the resolved absolute executable, executable digest, arguments, and entry artifact digest. Standalone Python/Node/Deno scripts are copied into owner-only content-addressed entry-file snapshots; scripts that need sibling modules must be installed as plugins. Installed plugin launchers snapshot and hash the bounded full plugin source tree so sibling-import drift also revokes approval. Launch uses the verified snapshot path and cwd. Registry package runners (`npx`, `uvx`, and `bunx`), `python -m`, shell/proxy launchers, and interpreter eval modes are rejected because they do not identify immutable executable bytes. Secret-bearing standalone interpreter scripts and interpreter aliases are rejected unless they resolve to a state-verified installed plugin tree snapshot; direct shebang executables with secret injection are rejected because they do not receive that tree binding. Raw JSON vault backend aliases (`json`, `file`, `local`, and the empty default alias) block every local stdio lifecycle and invocation path whenever the configured vault exists, regardless of its location. Keyring metadata with any secret record and workspace repair-signing/receipt material also block every local stdio lifecycle before secret resolution. A launch/material transition lock closes the race where a managed stdio session starts immediately before the first secret or receipt key is created; all registered local sessions must close before publication. This is lifecycle coupling inside the owning Kestrel runtime, not OCI or hostile-descendant containment. Use a remote authenticated MCP endpoint or a separately contained runtime for adversarial server code or independent Kestrel processes. A nonexistent raw-vault path and an empty keyring metadata file do not block and are never created or expanded by MCP launch. Dynamically discovered tools remain at least medium risk and approval-required even under `trust_manifest`; only reviewed static manifest metadata can declare an autonomous low-risk tool.
...
Agent-invoked test, lint, repair validation, and Codex delegation have no host-process path. Set `NEST_AGENT_VALIDATION_CONTAINER_IMAGE` to a preloaded immutable `name@sha256:<64 hex>` image containing the requested executable and dependencies. Kestrel copies the exact tracked-plus-untracked Git candidate through bounded no-follow opens into an owner-private snapshot, rejects sensitive/control paths, symlinks, hard links, special files, oversized trees, and candidate drift, then executes it with no network, no secrets, no host home, no Git or `.nest` state, a read-only source/root filesystem, nonroot identity, dropped capabilities, resource limits, bounded output, verified cleanup, and no host fallback. The runtime uses `--pull=never`; configuration and operator documentation must not substitute a mutable tag or a bare image ID. A passing repair-validation receipt includes that OCI attestation and the exact branch, HEAD, diff digest, image digest, and snapshot digest.
