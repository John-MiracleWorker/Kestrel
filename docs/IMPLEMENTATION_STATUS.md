...
- Controlled self-modification: behavior-delta schema, SQLite ledger persistence, proposal-only task capsule extraction, repeated failed tool-call heuristic extraction, and `nest-agent memory deltas propose --dry-run` exist for reviewable proposals. Mutation gating, replay validation, runtime behavior compilation, API/UI review surfaces, skill-candidate promotion, and ORACLE shadow integration remain future phases.
