...
- Controlled self-modification: behavior-delta schema, SQLite ledger persistence, proposal-only task capsule extraction, repeated failed tool-call heuristic extraction, `nest-agent memory deltas propose --dry-run`, rule-based mutation-gate decisions, a default-off behavior compiler, runtime context integration behind `NEST_AGENT_ENABLE_BEHAVIOR_DELTAS`, deterministic compiler replay, full mock-agent replay, `nest-agent memory deltas ledger` JSON reporting with advisory recommendations, preview-only skill-candidate rendering, ORACLE shadow-only behavior-delta counterfactual reporting, and read-only FastAPI behavior-delta review routes exist for reviewable proposals. Live provider replay, web UI review panels, mutating approval-gated review endpoints, and approval-gated skill install from candidates remain future phases.
