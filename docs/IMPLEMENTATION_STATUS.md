...
- SQLite state store for runs, run steps, approvals, MCP servers, skills, plugins, task nodes, subagent runs, trace spans, promotion ledger outcomes, behavior-delta ledger records, per-tool/MCP/skill capability overrides, and append-only capability changes, now initialized through schema version `15`.
...
- Runtime settings can persist `max_tool_rounds` through `/api/runtime/settings`; the Settings UI exposes this as “Max tool calls” for new runs.
