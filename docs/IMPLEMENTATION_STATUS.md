...
- Controlled self-modification: behavior-delta schema and SQLite ledger persistence exist for proposed/staged/active/rejected/rolled-back/expired deltas, activation records, outcome records, and summary rates. Proposal extraction, mutation gating, replay validation, runtime behavior compilation, CLI/API/UI review surfaces, skill-candidate promotion, and ORACLE shadow integration remain future phases.
