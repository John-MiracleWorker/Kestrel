...
- First-run setup readiness checks now inspect non-secret local prerequisites through `nest-agent product setup` and `GET /api/product/setup`, covering provider configuration, workspace, memory/state/log paths, dangerous-action gates, worker isolation, and local API auth without mutating setup state or exposing secrets.
