...
- Controlled self-modification: behavior-delta schema, SQLite ledger persistence, proposal-only task capsule extraction, repeated failed tool-call heuristic extraction, `nest-agent memory deltas propose --dry-run`, and rule-based mutation-gate decisions exist for reviewable proposals. Replay validation, runtime behavior compilation, API/UI review surfaces, skill-candidate promotion, and ORACLE shadow integration remain future phases.
