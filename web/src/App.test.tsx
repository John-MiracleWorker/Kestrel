...
describe("App", () => {
  ...
  it("reuses the same run-now idempotency key after an accepted request returns 500", async () => {
    const fetchSpy = vi.mocked(fetch);
    routineRunNowAmbiguousStatuses = [500];
    render(<App />);
    ...
    fireEvent.click(screen.getByRole("button", { name: "Retry Morning review now" }));
  ...
  it("treats an explicit failed repair validation as authoritative over artifact fallback", async () => {
    ...
    const repairRun: Run = {
      ...
      message: "Repair with failed validation",
      session_id: "session_repair_failed_validation",
      assistant_message: "Repair validation failed"
    };
    runs = [repairRun];
    sessions = [{
      session_id: repairRun.session_id,
      run_count: 1,
      status_counts: { completed: 1 },
      latest_run_id: repairRun.run_id,
      latest_status: repairRun.status,
      latest_message: repairRun.message,
      created_at: repairRun.created_at,
      updated_at: repairRun.updated_at
    }];
    sessionRuns = { [repairRun.session_id]: [repairRun] };
    approvals = [];
    taskGraphs[repairRun.run_id] = {
      tasks: [{
        task_id: "validate-failed",
        title: "Validate repair",
        goal: "Run targeted validation",
        profile: "worker",
        status: "completed",
        ...
        result: {
          ...
          repair_artifact: {
            ...
            repair_snapshot: {
              ...
              diff_digest: "bad0bad0bad0bad0bad0bad0bad0bad0bad0bad0bad0bad0bad0bad0bad0bad0"
            }
          }
        }
      }],
...
async function fetchMock(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
  ...
  if (routineRunNowMatch && init?.method === "POST") {
    ...
    const ambiguousStatus = routineRunNowAmbiguousStatuses.shift();
    if (ambiguousStatus !== undefined) {
      return jsonResponse({ detail: "routine_dispatch_outcome_unconfirmed" }, ambiguousStatus);
    }
    return jsonResponse({
