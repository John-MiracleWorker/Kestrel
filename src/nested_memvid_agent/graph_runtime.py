        ...
        with services.spans.start(
            ...
            if root is not None:
                ...
                existing_plan["graph_runtime"] = {
                    ...
                    "can_revise_semantic_plan": semantic_plan.get("source") == "provider_structured",
            ...
            services.events.publish(
                ...
                "orchestration.plan",
                {
                    ...
                    "provider_plan_status": provider_plan_status if root is not None else "no_root_task",
                    "can_rewrite_task_dag": False,
                },
            )
            span.set_result(
...
def evaluate_turn_review(
    *,
    ...
    if result.stop_reason != "complete" or result.error or unresolved_failures:
        ...
        return _terminal_review_failure(
            stop_reason=result.stop_reason if result.stop_reason != "complete" else "review_evidence_failed",
            error=error,
            result=result,
            root_task=root_task,
            additional_tool_executions=additional_tool_executions,
    ...
    if _provider_review_enabled(config, agent):
        ...
        if provider_artifact is not None:
            ...
            review: dict[str, Any] = {
                ...
                "gate": "provider_semantic_review" if passed else "provider_semantic_review_rejected",
                "artifact": provider_artifact,
                "provider_review_status": provider_review_status,
            }
            if not passed:
                review["error"] = str(provider_artifact.get("summary") or "Semantic review rejected completion")
            return review
...
def _deterministic_semantic_plan(
    *,
    ...
    if not criteria:
        criteria = ["The assistant returns a non-empty result without unresolved execution failures."]
    return {
        "schema_version": 1,
        "revision": revision,
    ...
    *,
    ...
    for item in raw_assessments:
        ...
        validation_refs = [
            ref
            for ref in refs
            if str(evidence_by_ref[ref].get("kind") or "") == "validation"
        ]
        if (
    ...
    return {
        ...
        "evidence": evidence,
        "remaining_risks": _safe_string_list(payload.get("remaining_risks"), limit=6, item_limit=300),
...
def _terminal_review_failure(
    *,
    ...
    executions = (*additional_tool_executions, *(result.tool_executions if result is not None else ()))
    artifact = _deterministic_review_artifact(
        result=result,
        root_task=root_task,
...
def _deterministic_review_artifact(
    *,
    ...
    resolved_decision = decision or ("pass" if all(item["satisfied"] for item in assessments) else "fail")
...
def _deterministic_criterion_assessment(
    ...
    *,
    ...
    for item in evidence:
        evidence_by_kind.setdefault(str(item.get("kind") or ""), []).append(str(item.get("id") or ""))
    ...
    elif any(term in normalized for term in ("objective", "addressed", "assistant", "result", "response")):
        refs = assistant_refs
...
def criterion_requires_validation_evidence(criterion: str) -> bool:
    ...
    return any(
        word.startswith(("check", "lint", "test", "validat", "verif"))
        for word in words
    )
...
def _semantic_review_criteria(root_task: TaskNodeRecord | None) -> list[str]:
    ...
    return deduplicated or ["The assistant returns a non-empty result without unresolved execution failures."]
