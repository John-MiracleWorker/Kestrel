...
def provider_config_digest(
    ...
    # The material contains only configuration identifiers, never a credential value.
    # Its credential-environment identifier is nevertheless treated as sensitive:
    # PBKDF2 keeps the fingerprint deterministic and exact without applying a fast
    # general-purpose hash to potentially sensitive configuration metadata.
    return hashlib.pbkdf2_hmac(
        "sha256",
        encoded,
        b"kestrel-provider-config-v1",
        100_000,
    ).hex()
