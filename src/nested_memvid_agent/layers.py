...
def _load_or_create_memory_integrity_key(memory_dir: Path) -> bytes:
    ensure_private_directory(memory_dir)
    descriptor = open_private_file_descriptor(memory_dir / _MEMORY_INTEGRITY_KEY_LOCK_FILE)
    try:
...
class LayeredMemorySystem:
    ...
    def __init__(
        ...
        self._unsettled_tool_execution_lock = Lock()
        self._unsettled_tool_execution_ids: set[str] = set()
    ...
    def retain_for_unsettled_tool_execution(self, execution_id: str) -> None:
        """Prevent backend teardown while an indeterminate tool can still resume."""

        with self._unsettled_tool_execution_lock:
            self._unsettled_tool_execution_ids.add(execution_id)

    def release_unsettled_tool_execution(self, execution_id: str) -> None:
        """Release a tool resource fence only after every worker has quiesced."""

        with self._unsettled_tool_execution_lock:
            self._unsettled_tool_execution_ids.discard(execution_id)

    def has_unsettled_tool_executions(self) -> bool:
        with self._unsettled_tool_execution_lock:
            return bool(self._unsettled_tool_execution_ids)

    def close_all(self) -> None:
        # A timed-out Python thread cannot be killed safely. Never close or
        # release an exclusive Memvid owner while that thread (or its untrusted
        # cancellation hook) can still resume against these handles.
        if self.has_unsettled_tool_executions():
            raise MemoryCleanupIncompleteError(
                (self,),
                phase="unsettled_tool_execution",
            )
    ...
    if authority == "nested_learning":
        ...
        if not isinstance(validation, dict):
            ...
            if not isinstance(resolution_ids, list) or not resolution_ids:
                ...
                target_run_id = str(raw_target_run_id) if raw_target_run_id is not None else None
                for resolution_id in (str(item) for item in resolution_ids):
                    ...
                    if (
                        ...
                        break
                    if (binding[3] is not None and binding[3] != target_run_id) or (
                        binding[3] is None and binding[2] != target_session_id
                    ):
            ...
            if (
                ...
                or any(
                    ...
                    or (
                        ...
                        and str(binding.get("source") or "").strip() != "repair.validate"
                    )
                    or (
                        str(binding.get("evidence_bucket") or "") == "review"
                        and str(binding.get("source") or "").strip() != "repair.review"
                    )
                    or (
