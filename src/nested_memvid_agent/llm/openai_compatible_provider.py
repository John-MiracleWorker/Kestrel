        ...
        try:
            for chunk in client.chat.completions.create(**request):
                ...
                if choice is None:
                    continue
                finish_reason = str(
                    getattr(choice, "finish_reason", finish_reason) or finish_reason or ""
                )
    ...
    for index, item in enumerate(raw_calls):
        ...
        calls.append(
            ToolCall(
                name=name,
                arguments=arguments,
                id=native_tool_call_id(_item_value(item, "id"), location=f"{location}.id"),
            )
        )
    return calls
...
def _accumulate_delta_tool_calls(buffers: dict[int, dict[str, str]], raw_calls: Any) -> None:
    ...
    for position, item in enumerate(raw_calls):
        ...
        if item_id is not None:
            validated_id = native_tool_call_id(
                item_id,
                location=f"{location}.id",
                required=True,
            )
            if buffer["id"] and buffer["id"] != validated_id:
                raise ControlMessageError(
                    f"{location}.id conflicts with an earlier stream fragment",
                    code="invalid_tool_call_id",
                )
            buffer["id"] = validated_id
    ...
    for index, item in sorted(buffers.items()):
        ...
        calls.append(
            ToolCall(
                name=name,
                arguments=arguments,
                id=native_tool_call_id(item["id"] or None, location=f"{location}.id"),
            )
        )
    return calls
