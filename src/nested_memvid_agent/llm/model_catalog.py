...
def model_catalog_for_provider(config: AgentConfig, provider: str) -> ProviderModelCatalog:
    ...
    try:
        return _fetch_provider_models(config, provider_name, fallback)
        ...
        return ProviderModelCatalog(
            ...
            api_key_configured=_api_key_configured(config, provider_name),
        )


def all_model_catalogs(config: AgentConfig) -> list[ProviderModelCatalog]:
...
def _fetch_provider_models(config: AgentConfig, provider: str, fallback: tuple[str, ...]) -> ProviderModelCatalog:
    if provider == "openai":
        api_key_env = _api_key_env_for_provider(config, provider)
        ...
        return _catalog(provider, _model_ids(payload), fallback, "provider", None, api_key_env)
    if provider == "openai-compatible":
        if not config.base_url:
            raise ValueError("openai-compatible provider requires NEST_AGENT_BASE_URL or --base-url")
        payload = _fetch_json(
            _join_url(config.base_url, "models"),
            timeout_seconds=_catalog_timeout(config),
            api_key=_optional_api_key(config.api_key_env),
        )
        return _catalog(provider, _model_ids(payload), fallback, "provider", config.base_url, config.api_key_env)
    if provider == "openrouter":
        api_key_env = _api_key_env_for_provider(config, provider)
        base_url = _base_url_for_provider(config, provider) or "https://openrouter.ai/api/v1"
        payload = _fetch_json(
            _join_url(base_url, "models"),
            timeout_seconds=_catalog_timeout(config),
            api_key=_optional_api_key(api_key_env),
        )
        return _catalog(provider, _model_ids(payload), fallback, "provider", base_url, api_key_env)
    if provider == "deepseek":
        api_key_env = _api_key_env_for_provider(config, provider)
        api_key = _required_api_key(api_key_env)
        base_url = _base_url_for_provider(config, provider) or "https://api.deepseek.com"
        payload = _fetch_json(
            _join_url(base_url, "models"),
            timeout_seconds=_catalog_timeout(config),
            api_key=api_key,
    ...
    if provider == "kimi":
        api_key_env = _api_key_env_for_provider(config, provider)
        api_key = _required_api_key(api_key_env)
        base_url = _base_url_for_provider(config, provider) or "https://api.moonshot.ai/v1"
        payload = _fetch_json(
            _join_url(base_url, "models"),
            timeout_seconds=_catalog_timeout(config),
            api_key=api_key,
        ...
        return _catalog(provider, _model_ids(payload), fallback, "provider", base_url, api_key_env)
    if provider == "ollama":
        base_url = _base_url_for_provider(config, provider) or "http://localhost:11434/v1"
        payload = _fetch_json(
            _join_url(base_url, "models"),
            timeout_seconds=_catalog_timeout(config),
            api_key=None,
        )
        return _catalog(provider, _model_ids(payload), fallback, "provider", base_url, None)
    if provider == "ollama-cloud":
        api_key_env = _api_key_env_for_provider(config, provider)
        api_key = _required_api_key(api_key_env)
        base_url = _base_url_for_provider(config, provider) or "https://ollama.com/api"
        payload = _fetch_json(
...
def _required_api_key(api_key_env: str | None) -> str:
    if not api_key_env:
        raise ValueError("provider API key environment variable is not configured")
...
def _optional_api_key(api_key_env: str | None) -> str | None:
    return os.getenv(api_key_env) if api_key_env else None


def _api_key_configured(config: AgentConfig, provider: str) -> bool:
    api_key_env = _api_key_env_for_provider(config, provider)
