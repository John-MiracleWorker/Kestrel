...
def _agent_has_unsettled_tool_executions(agent: Any) -> bool:
    """Capability-safe check for real agents and lifecycle test doubles."""

    memory = getattr(agent, "memory", None)
    checker = getattr(memory, "has_unsettled_tool_executions", None)
...
class RunManager:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            self._failed_agent_closures: dict[int, tuple[str | None, NestedMV2Agent]] = {}
            self._unsettled_tool_agents: dict[int, tuple[str | None, NestedMV2Agent]] = {}
    ...
    def _run_lease(self, run_id: str, config: AgentConfig) -> Iterator[RunRecord | None]:
        ...
        with execution_lock:
            ...
            with self._operation_condition:
                ...
                self._active_run_operations[run_id] = self._active_run_operations.get(run_id, 0) + 1
            try:
            ...
            try:
                ...
                if agent is not None:
                    try:
                        ...
                        agent = None
                error_text = str(redact_secrets(f"{type(error_exc).__name__}: {error_exc}"))
                cancelled = (
    ...
    def _execute_approved_tool(
        ...
        *,
        ...
        def heartbeat_claim() -> None:
            while not heartbeat_stop.wait(heartbeat_interval):
                try:
                    ...
                    renewed = False
                # Renewal can block in storage while the tool completes.  The
                # stop signal must therefore be re-read *after* the call, with
                # the cancellation decision serialized against finalization.
                # Once finalization sets stop, this thread can never issue a
                # late process-tree cancellation.
                with heartbeat_decision_lock:
        ...
        def stop_claim_heartbeat() -> None:
            heartbeat_stop.set()
            # Wait for any already-admitted cancellation decision to finish.
            # Do not wait for a blocked storage renewal: the daemon will see
            # the stop signal when that bounded/external operation returns.
            with heartbeat_decision_lock:
                pass
        ...
        try:
            ...
            # Fence heartbeat cancellation before persisting a terminal error
            # receipt; finalized executions must never be cancelled later.
        ...
        outcome_indeterminate = bool(
            isinstance(payload_data, dict) and payload_data.get("outcome_indeterminate") is True
        )
        if outcome_indeterminate:
            # Persisting the exact non-retryable receipt is the durable
            # at-most-once fence. Leaving the claim open would let its lease
            # expire and admit a duplicate while the daemon worker may still
            # commit. The receipt closes execution admission, but explicitly
            # remains unresolved until an operator reconciles the side effect.
            assert isinstance(payload_data, dict)
            payload["data"] = {
                **payload_data,
                "approval_execution_state": "reconciliation_required",
                "execution_claim_finalized_to_prevent_replay": True,
            }
        ...
        if outcome_indeterminate:
            self.events.publish(
                run_id,
                "approval.execution_outcome_indeterminate",
                payload,
            )
            ...
            with self._worker_heartbeat(
                ...
                with self._scheduler_approval_scope(
                    ...
                    result = agent.chat(
                        ...
                        turn_origin="scheduler_task",
                        transcript_scope="internal",
                        execution_origin=f"subagent:{subagent.subagent_id}",
                    )
            if (
                worker_lost.is_set()
    ...
    def _build_agent(self, config: AgentConfig) -> NestedMV2Agent:
        ...
        try:
            ...
            if release_memvid_slot is not None:
                with self._lock:
                    self._quarantined_memory_cleanups.append((exc, release_memvid_slot))
            raise
        except BaseException:
            if release_memvid_slot is not None:
                release_memvid_slot()
    ...
    def close_runtime_agent(
        ...
        *,
        ...
        if _agent_has_unsettled_tool_executions(agent):
            # Returning an indeterminate tool receipt must not be followed by
            # closing handles which its daemon worker can still resume against.
            # Retain both the agent and its Memvid slot, then retry only after
            # the registry's quiescence watcher has released the memory fence.
            with self._lock:
                self._unsettled_tool_agents[id(agent)] = (run_id, agent)
            return
        ...
        try:
            ...
            with self._lock:
                ...
                self._unsettled_tool_agents.pop(id(agent), None)
    ...
    def _retry_failed_memory_cleanup(self) -> bool:
        ...
        with self._lock:
            ...
            unsettled_tool_agents = tuple(self._unsettled_tool_agents.items())
            construction_cleanups = tuple(self._quarantined_memory_cleanups)

        for agent_id, (run_id, agent) in unsettled_tool_agents:
            if _agent_has_unsettled_tool_executions(agent):
                continue
            try:
                agent.close()
                ...
                with self._lock:
                    self._unsettled_tool_agents.pop(agent_id, None)
                    self._failed_agent_closures[agent_id] = (run_id, agent)
                continue
            with self._lock:
                retained = self._unsettled_tool_agents.get(agent_id)
        ...
        with self._lock:
            return (
                not self._failed_agent_closures
                and not self._unsettled_tool_agents
                and not self._quarantined_memory_cleanups
            )
    ...
    def build_registry(self, config: AgentConfig | None = None) -> ToolRegistry:
        ...
        registry = build_default_tools(
            active_config.enabled_tools,
            runtime_fence=self._tool_fence,
        )
        for adapter in [
    ...
    def operational_counters(self) -> dict[str, int]:
        with self._lock:
            return {
                ...
                "cancelled_run_durability_failures": (self._cancelled_run_durability_failure_count),
                "admission_reconciliation_failures": (self._admission_reconciliation_failure_count),
                "admission_reconciliations_pending": len(self._failed_admission_reconciliations),
                "oci_container_cleanups_pending": (self.skills.pending_container_cleanup_count),
    ...
    def _run_primary_thread(
        ...
        try:
            try:
                ...
                # Shutdown can win immediately after Thread.start() but before
                # the target registers its run-lease operation. The run has
                # already been included in shutdown cancellation; do not leak
                # that expected admission rejection as an unhandled daemon
                # thread exception.
                if str(exc) != "run_manager_shutting_down" or not self._shutdown_event.is_set():
                    raise
        finally:
            try:
...
def _workspace_supports_git_worktree(workspace: Path) -> bool:
    ...
    completed = subprocess.run(  # noqa: S603 - fixed git executable and structured argv  # nosec
        hardened_readonly_git_command(["rev-parse", "--show-toplevel"], workspace=root),
        cwd=root,
            ...
            payload.update(
                {
                    ...
                    "requested_stable_layer": staged_record.metadata["requested_stable_layer"],
