    ...
    *,
    ...
    try:
        completed = subprocess.run(  # noqa: S603 - fixed read-only git commands  # nosec
            _hardened_git_command(_git_command_arguments(command), workspace=context.workspace),
    ...
    def run(self, arguments: dict[str, Any], context: ToolContext) -> ToolExecution:
        ...
        if result.success:
            try:
                ...
                head_sha = _git_output(context.workspace, ["git", "rev-parse", "HEAD"])
    ...
    def run(self, arguments: dict[str, Any], context: ToolContext) -> ToolExecution:
        ...
        if path_arg:
            try:
                ...
                _assert_workspace_path_allowed(context, path, requested_path=path_arg)
        ...
        try:
            ...
            if completed.returncode == 0:
                sha = subprocess.run(  # nosec
                    _hardened_git_command(["rev-parse", "HEAD"], workspace=context.workspace),
                    cwd=context.workspace,
                    env=_sanitized_git_environment(None),
                    capture_output=True,
                    text=True,
...
def _current_index_tree_sha(workspace: Path) -> str:
    ...
    if completed.returncode != 0:
        raise RuntimeError(redact_text(f"Unable to preview staged Git tree: {completed.stderr}"))
...
def _decoded_failure(
    ...
    return subprocess.CompletedProcess(
        ...
        stderr=redact_text(f"{prefix}\n{completed.stderr.decode('utf-8', errors='replace')}"),
    )


def _completed_failure(command: str, message: str) -> subprocess.CompletedProcess[str]:
    return subprocess.CompletedProcess(command.split(), 1, stdout="", stderr=redact_text(message))
