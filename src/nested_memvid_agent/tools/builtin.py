...
class MemoryPolicyPromoteTool(AgentTool):
    ...
    def run(self, arguments: dict[str, Any], context: ToolContext) -> ToolExecution:
        ...
        try:
            ...
            record = kernel.to_memory_record(signal, decision)
            source_record_ids = tuple(dict.fromkeys((source_record_id, *source_record_ids)))
            record.evidence.append(EvidenceRef(source="memory_record", locator=source_record_id))
            record_id = (
                None
                if dry_run
                else context.memory.put_validated(
    ...
    *,
    ...
    for bucket, refs in (
        ...
        for ref in refs:
            ...
            if (context.run_id is not None and receipt_run_id != context.run_id) or (
...
def _resolve_capsule_summary_evidence(summary: Any, *, context: ToolContext) -> Any:
    ...
    for signal in summary.learning_signals:
        ...
        if evidence is None:
            ...
            continue
        expected_subject_id = str((signal.metadata or {}).get("source_record_id") or "").strip()
