...
def _memory(root: Path, *, ledger: PromotionLedger | None = None) -> LayeredMemorySystem:
    return LayeredMemorySystem.from_backend_factory(root / "memory", InMemoryBackend, ledger=ledger)
...
def _case_retrieval_contract(root: Path) -> dict[str, Any]:
    ...
    for layer in MemoryLayer:
        memory.put(
            MemoryRecord(
                ...
                confidence=max(DEFAULT_LAYER_SPECS[layer].min_write_confidence, 0.98 if layer == MemoryLayer.POLICY else 0.85),
            )
        )
    ...
    return {
        "passed": set(layers) == {layer.value for layer in MemoryLayer} and last_retrieved_count == len(MemoryLayer),
...
def _case_context_summary_first(root: Path) -> dict[str, Any]:
    ...
    _put(memory, MemoryLayer.EPISODIC, "Eval summary", f"{sentinel} summary.", frame_type="task_summary")
    _put(memory, MemoryLayer.EPISODIC, "Eval raw", f"{sentinel} raw exact evidence.", frame_type="raw_chunk")
    _put(memory, MemoryLayer.EPISODIC, "Eval correction", f"{sentinel} correction.", kind=MemoryKind.CORRECTION, frame_type="correction")
    compact = ContextPacker(memory).pack(ContextPackRequest(objective=sentinel, query=sentinel))
    exact = ContextPacker(memory).pack(ContextPackRequest(objective=f"Need exact quote for {sentinel}", query=sentinel))
    compact_titles = [item.frame.title for item in compact.items]
    exact_titles = [item.frame.title for item in exact.items]
    return {
        "passed": "Eval summary" in compact_titles and "Eval raw" not in compact_titles and "Eval raw" in exact_titles,
...
def _case_promotion_gates(root: Path) -> dict[str, Any]:
    ...
    return {
        "passed": not procedural_one.accepted and procedural_two.accepted and not policy_event.accepted,
...
def _case_correction_tombstone(root: Path) -> dict[str, Any]:
    ...
    return {
        "passed": bool(result.success and target and target.metadata.get("active") is False and row.outcome_counts["corrected"] == 1),
...
def _case_promotion_ledger(root: Path) -> dict[str, Any]:
    ...
    ledger = PromotionLedger(AgentStateStore(state_path))
    memory = _memory(root, ledger=ledger)
    record_id = memory.put(_promoted_record("eval-ledger-promotion", "Eval ledger fact", "eval_ledger_44 useful."))
...
def _case_agent_cross_layer_flow(root: Path) -> dict[str, Any]:
    ...
    agent = NestedMV2Agent(
        AgentDependencies(
            ...
            llm=MockLLMProvider(
                [
                    LLMResponse(content="Capturing.", tool_calls=(ToolCall(name="eval.observe", arguments={}),)),
    ...
    memory.put(kernel.to_memory_record(signal, kernel.decide(signal)))
    compiled = ContextCompiler(memory, config=ContextCompilerConfig(context_pack_token_budget=650)).compile(
        "eval_cross_layer_66 later objective",
        query="eval_cross_layer_66 semantic fact",
    )
    return {
        "passed": "Eval cross-layer fact" in compiled.prompt and len(list(memory.iter_records(MemoryLayer.POLICY))) == 0,
...
def _case_backend_consistency(root: Path) -> dict[str, Any]:
    ...
    backend.seal()
    return {
        "passed": bool(hits_before and not hits_after and inactive_hits and backend.verify() and path.with_suffix(".memory.json").exists()),
...
def _promoted_record(promotion_id: str, title: str, content: str) -> MemoryRecord:
    return MemoryRecord(
        ...
        metadata={
            ...
            "explicit_instruction": False,
            "nested_learning": {
                "context_flow": {"source_layers": ["episodic"]},
                "decision": {"reason": "eval promotion"},
