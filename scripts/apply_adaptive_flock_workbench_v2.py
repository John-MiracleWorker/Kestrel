...
def main() -> None:
    ...
    advanced_marker = '            {activeSection === "advanced" && (\n              <section\n'
    text = replace_once(
        text,
        advanced_marker,
        routing_section + advanced_marker,
    ...
    test_text = replace_once(
        ...
        routing_test + '  it("keeps idle chat polling lightweight", async () => {\n',
        name="routing integration test",
    )

    routing_fixtures = indent(
        dedent(
            '''\
    ...
    for path in (
        Path(".github/workflows/adaptive-flock-workbench.yml"),
        Path("scripts/apply_adaptive_flock_workbench.py"),
        Path(__file__),
    ):
        if path.exists():
            path.unlink()
