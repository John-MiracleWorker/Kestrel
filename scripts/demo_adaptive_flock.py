...
Production-truth reconciliation (S11 / PR #311): the demo is a
*contract-level* exercise.  In the shipped v0.6 runtime, learned routing is
inert by design -- neither ``server.py`` nor ``cli.py`` wires an activation
evaluator, so every real decision falls back to the deterministic static
path with ``durable_grant_required``.  The ``constrained``/``adaptive``
modes shown here describe the routing contracts only; they are not wired
into production execution.  The only v0.6 learned-authority class is an
exact, owner-activated, low-risk summarizer scope (AUTH-002), and this demo
...
def production_truth_block() -> dict[str, Any]:
    """The truthful production authority state the demo does not alter.

    Kept in one place so the JSON report and the docs can never drift from
    the code.  Learned routing is not wired into the production runtime; a
    live learned-authority grant would require an exact owner-activated
    low-risk summarizer scope (AUTH-002) plus a wired activation evaluator.
    """
    return {
        "wired_activation_evaluator": False,
        "live_grant": False,
        "learned_authority_class": "v06_low_risk_summarizer_only",
        "deterministic_fallback_reason": "durable_grant_required",
        "claim": (
            "contract-level routing exercise only; grants no production "
            "execution authority"
        ),
    }
...
def build_demo_report(mode: RoutingMode = "shadow") -> dict[str, Any]:
    ...
    return {
        ...
        "production_truth": production_truth_block(),
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default="shadow",
        help=(
            "Contract-level routing mode to report. Shadow is the safe, "
            "non-actionable default; no mode wires learned routing into the "
            "production runtime."
        ),
    )
...
def _render_text(report: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "Production truth: learned routing is NOT wired into the v0.6 runtime",
            f"  wired_activation_evaluator={str(report['production_truth']['wired_activation_evaluator']).lower()}",
            f"  live_grant={str(report['production_truth']['live_grant']).lower()}",
            f"  fallback={report['production_truth']['deterministic_fallback_reason']}",
        ]
    )
    return "\n".join(lines)
