...
OPTIONAL_RELEASE_ARTIFACTS = {"oci-image-digests.json"}
...
def _verify_hash_locked_requirements(path: Path) -> int:
    ...
    for requirement in requirements:
        ...
        if "==" not in requirement_spec or _HASH_OPTION not in requirement:
            raise ValueError(
                f"requirement is not exact and hash-locked: {requirement_spec.strip()}"
            )
...
def _release_version(value: str) -> str:
    ...
    if (
        not version
        or version != value.lstrip("v")
        or not re.fullmatch(r"[0-9]+(?:\.[0-9]+)+(?:[a-zA-Z0-9.+-]*)?", version)
    ):
...
def _assert_identity(
    *,
    ...
    if _canonical_distribution(actual_distribution) != _canonical_distribution(
        ...
        raise ValueError(
            f"{label} distribution mismatch: {actual_distribution!r} != {expected_distribution!r}"
        )
    if actual_version != expected_version:
        raise ValueError(f"{label} version mismatch: {actual_version!r} != {expected_version!r}")
...
def _verify_sbom_identity(path: Path, *, expected_distribution: str, expected_version: str) -> None:
    try:
...
def verify_release_payload(
    ...
    *,
    ...
    allowed = required | set(wheels) | set(sdists) | OPTIONAL_RELEASE_ARTIFACTS
    unexpected = sorted(set(files) - allowed)
    if unexpected:
        raise ValueError(f"release payload contains unexpected artifacts: {unexpected}")
