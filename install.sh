...
release_install_exit_trap() {
  ...
  log "Kestrel install failed; restoring the previous checkout and Python environment."
...
control_paths = [Path(value) for value in sys.argv[2:]]
...
acquire_maintenance_lock() {
  ...
  local runtime_lock_path="${KESTREL_HOME}/.nest/state/.agent.db.kestrel-runtime-owner.lock"
...
stage_candidate_state_isolated() {
  local live_state="${KESTREL_HOME}/.nest/state/agent.db"
  if is_true "${KESTREL_DRY_RUN:-}"; then
...
cleanup_failed_server_launch() {
  ...
  if [[ -z "$supervisor_pid" ]]; then
    log "ERROR: server launch was attempted, but no supervisor identity was captured; process absence cannot be proven automatically."
    return 1
  fi
  ...
  if [[ -z "$server_pgid" ]]; then
    log "ERROR: failed launch never yielded a verified server process group; descendant absence cannot be proven."
    return 1
  fi
...
start_server_detached() {
  ...
  if ! launch_standard_server_detached; then
    if cleanup_failed_server_launch; then
      die "The filesystem install committed, but the Kestrel server could not be launched; the failed supervisor and child were stopped."
    fi
    die "The filesystem install committed, but server launch failed and process absence could not be proven. Inspect ${KESTREL_SERVER_SUPERVISOR_PID}, ${KESTREL_SERVER_PID}, and port ${KESTREL_PORT} before retrying."
  fi
  if ! wait_for_server_status; then
    if cleanup_failed_server_launch; then
      die "The filesystem install committed, but the Kestrel server did not become healthy and was stopped. Check ${KESTREL_SERVER_LOG}."
    fi
    die "The filesystem install committed, but server health failed and automatic cleanup could not prove the supervisor, child, and port absent. Manual recovery is required."
  fi
  SERVER_LAUNCH_ATTEMPTED=0
}
...
main() {
  ...
  commit_staged_web_assets
  commit_staged_memory
  # A real server may recover durable work as soon as it starts, so it is not a
  # passive pre-commit health canary. Commit the verified filesystem install
  # first, then launch exactly one real service without rollback underneath it.
  finalize_release_install_transaction
  finish_post_commit_maintenance
  start_server_detached
  open_web_ui_best_effort
